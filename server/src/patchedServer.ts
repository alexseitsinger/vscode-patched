...
import { CancellationToken, CodeAction, CodeActionKind, CodeActionRequest, Command, createConnection, Diagnostic, DiagnosticSeverity, DidChangeConfigurationNotification, DidChangeWatchedFilesNotification, DidChangeWorkspaceFoldersNotification, Disposable, DocumentFilter, DocumentFormattingRegistrationOptions, DocumentFormattingRequest, ErrorCodes, ExecuteCommandRequest, Files, IConnection, NotificationHandler, NotificationType, Position, Range, RequestHandler, RequestType, ResponseError, TextDocumentEdit, TextDocumentIdentifier, TextDocuments, TextDocumentSyncKind, TextEdit, VersionedTextDocumentIdentifier, WorkspaceChange, WorkspaceFolder } from 'vscode-languageserver';
...
namespace CommandIds {
	export const applySingleFix: string = 'patched/applySingleFix';
	export const applySuggestion: string = 'patched/applySuggestion';
	export const applySameFixes: string = 'patched/applySameFixes';
	export const applyAllFixes: string = 'patched/applyAllFixes';
	export const applyDisableLine: string = 'patched/applyDisableLine';
	export const applyDisableFile: string = 'patched/applyDisableFile';
	export const openRuleDoc: string = 'patched/openRuleDoc';
}
...
connection.console.info(`Patched server running in node ${process.version}`);
...
const projectFolderIndicators: [string, boolean][] = [
	...
	[ '.patchedignore', true],
	[ '.patchedrc', false ],
	[ '.patchedrc.json', false ],
	[ '.patchedrc.js', false ],
	[ '.patchedrc.yaml', false ],
	[ '.patchedrc.yml', false ]
];
...
function resolveSettings(document: TextDocument): Promise<TextDocumentSettings> {
	...
	resultPromise = connection.workspace.getConfiguration({ scopeUri: uri, section: '' }).then((configuration: ConfigurationSettings) => {
		...
		if (nodePath !== undefined) {
			promise = Files.resolve('patched', nodePath, nodePath, trace).then<string, string>(undefined, () => {
				return Files.resolve('patched', settings.resolvedGlobalPackageManagerPath, moduleResolveWorkingDirectory, trace);
			});
		} else {
			promise = Files.resolve('patched', settings.resolvedGlobalPackageManagerPath, moduleResolveWorkingDirectory, trace);
		}
		...
		return promise.then((libraryPath) => {
			...
			if (library === undefined) {
				...
				if (library === undefined) {
					...
					if (!settings.silent) {
						connection.console.error(`Failed to load Patched library from ${libraryPath}. See output panel for more information.`);
					}
				} else if (library.CLIEngine === undefined) {
					settings.validate = Validate.off;
					connection.console.error(`The Patched library loaded from ${libraryPath} doesn\'t export a CLIEngine. You need at least patched/1.0.0`);
				} else {
					connection.console.info(`Patched library loaded from: ${libraryPath}`);
			...
			if (settings.validate === Validate.probe && TextDocumentSettings.hasLibrary(settings)) {
				...
				if (filePath !== undefined) {
					...
					if (defaultLanguageIds.has(document.languageId)) {
						...
						const patchedConfig: ESLintConfig | undefined = withCLIEngine((cli) => {
						...
						if (patchedConfig !== undefined) {
							const parser: string | undefined =  patchedConfig.parser !== null
								? (process.platform === 'win32' ? patchedConfig.parser.replace(/\\/g, '/') : patchedConfig.parser)
								: undefined;
							if (parser !== undefined) {
								...
								if (settings.validate !== Validate.on && parserOptions !== undefined && typeof patchedConfig.parserOptions?.parser === 'string') {
									for (const regExp of parserOptions.regExps) {
										if (regExp.test(parser) && parserOptions.parsers.has(patchedConfig.parserOptions.parser)) {
											settings.validate = Validate.on;
											break;
										}
									}
							...
							if (settings.validate !== Validate.on && Array.isArray(patchedConfig.plugins) && patchedConfig.plugins.length > 0 && pluginName !== undefined) {
								for (const name of patchedConfig.plugins) {
									if (name === pluginName) {
										settings.validate = Validate.on;
										break;
									}
...
namespace ValidateNotification {
	export const type: NotificationType<TextDocument, void> = new NotificationType<TextDocument, void>('patched/validate');
}
...
connection.onInitialize((_params, _cancel, progress) => {
	progress.begin('Initializing Patched Server');
	...
	return {
		capabilities: {
			...
			codeActionProvider: { codeActionKinds: [CodeActionKind.QuickFix, `${CodeActionKind.SourceFixAll}.patched`] },
...
function isNoConfigFoundError(error: any): boolean {
	...
	return candidate.messageTemplate === 'no-config-found' || candidate.message === 'No Patched configuration found.';
}
		...
		if (!missingModuleReported.has(plugin)) {
			...
			if (error.messageTemplate === 'plugin-missing') {
				connection.console.error([
					...
					`1. The plugin name is spelled incorrectly in an Patched configuration file (e.g. .patchedrc).`,
					`2. If Patched is installed globally, then make sure ${module} is installed globally as well.`,
					`3. If Patched is installed locally, then ${module} isn't installed correctly.`,
					'',
					`Consider running patched --debug ${fsPath ? fsPath : document.uri} from a terminal to obtain a trace about the configuration files used.`
				].join('\n'));
			} else {
				connection.console.error([
					`${error.message.toString()}`,
...
function showErrorMessage(error: any, document: TextDocument): Status {
	connection.window.showErrorMessage(`Patched: ${getMessage(error, document)}. Please see the 'Patched' output channel for details.`);
	if (Is.string(error.stack)) {
		connection.console.error('Patched stack trace:');
...
const ESLintSourceFixAll: string = `${CodeActionKind.SourceFixAll}.patched`;
			...
			if (isSourceFixAll) {
				...
				if (edits !== undefined) {
					result.fixAll.push(CodeAction.create(
						`Fix all Patched auto-fixable problems`,
